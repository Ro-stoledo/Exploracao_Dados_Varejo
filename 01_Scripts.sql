...
SELECT COUNT(*) FROM SQL01.vendas;
SELECT ROUND(AVG(Quantidade),2) FROM SQL01.vendas;


-- 2. Qual o Número Total de Produtos Únicos Vendidos?
SELECT COUNT(DISTINCT Id_Produto) FROM SQL01.vendas;
...
FROM SQL01.vendas v
JOIN SQL01.produtos p ON v.Id_Produto = p.Id_Produto
GROUP BY p.nome
...
FROM SQL01.vendas v
JOIN SQL01.clientes c ON v.Id_Cliente = c.Id_Cliente
GROUP BY c.nome
...
WITH clientes_smartphone AS (
    SELECT Id_Cliente
    FROM SQL01.vendas
    JOIN SQL01.produtos ON vendas.Id_Produto = produtos.Id_Produto
...
FROM SQL01.clientes c
LEFT JOIN SQL01.vendas v ON c.Id_Cliente = v.Id_Cliente
